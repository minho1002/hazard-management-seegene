...
// /admin/user-audit(구 독립 페이지)에서 이동한 컴포넌트 — 로직은 그대로, /admin/system-history의
// "계정 변경 이력" 탭에서 렌더링된다.
export default function UserAuditPanel() {
  ...
  // 로그인 성공/실패는 별도의 "로그인 이력" 탭에서 확인하므로 여기서는 계정 변경 이벤트만 표시한다.
  const all = [...auditLogs]
  ...
  return (
    <div>
      <div style={{ padding: '20px 32px 16px', borderBottom: '1px solid #e3e8ef', background: '#fff' }}>
        ...
        <div style={{ fontSize: '0.72rem', color: '#697386', marginTop: 2 }}>표시 {sorted.length}건 / 전체 {all.length}건 · 로그인 이력은 별도 탭에서 확인</div>
      </div>
